...
The ability of state legislatures to use redistricting to influence the outcome of election comes at the direct expense of voter's ability to use their ballot to influence the outcome of elections.
...
These standards are attractive because they based on statistics with properties that are well understood, that are used broadly for applications across many fields, and are relatively simple.
...
The drawback of Monte Carlo based standards is complexity, in fact, despite the fact that Monte Carlo simulation is a common and highly conventional technique, the supreme court has expressed skepticism of this type of approach CITE.
Whether or not a court can be convinced to warm up to such an approach has yet to be determined, but what is clear is that monte carlo based methods are powerful tools for examination of bias.
...
In fact, the empirical Bayesian model that we propose is compatible with all of the metrics we have discussed thus far, and can be used as a part of a standard itself, or can be used to test the robustness of standards that do not require Monte Carlo simulation. 
We therefore contend that regardless of whether or not the approach we present is acceptable judicial standard itself, it remains a powerful tool for assessing partisan bias in redistricting plans that will prove useful for researchers and advocates of redistricting reform.
...
Additionally, the technique could be used to help develop an understanding of the conditions where different metric may agree or disagree with one another.
...
First, single winner elections tend not to produce outcomes CITE, and we therefore can not use proportionality or the lack thereof as a measure of bias.
...
To address these challenges, we propose a measure of bias which we call the specific asymmetry, which is the deviation of the seats votes curve from the asymmetry line measured at the statewide popular vote.
...
For example, the plaintiffs in \emph{Whitford v. Gil} argued that based on an examination of a large body of election data, and observed efficiency gap of greater than 7\% in a given election was strongly suggestive of that bias in favor of the redistricting party would persist.
...
Analysis of historic data is not the only way to asses likely future harm due to bias, and we would like to use a method that takes advantage of as much specific information about the redistricting plan in question as possible.
...
The expected net asymmetry for each redistricting cycle is also shown with 25 and 5\% confidence bounds.
A progression is observed where the net asymmetry strongly favored democrats in the 70's and 80's, mildly favoring democrats in the 90's, was roughly zero net asymmetry in the 2000's, to strongly favoring republicans in the 2010 cycle.
...
In the 80's for example, the Democrats could expect about 20 seats on average due to redistricting bias, with a negligibly small probability that the net national asymmetry would favor Republicans.
...
For earlier cycles, we observe qualitative agreement with similar studies CITE in terms which party benefited on net from gerrymandering.
...
National voting results are used as a baseline, but this baseline may contain substantial bias which means only large gerrymanders will be successfully identified. 
...
Democrats obtained seats due to small levels of bias in a number of states, and additionally obtained significant numbers of seats in larger states like California and Texas.
...
In the 80's Texas has 24 seats and  California had 43, Pennsylvania had 18 seats in the 2010s and North Carolina had 13.
...
Regardless of which party benefited from bias on balance, bias in redistricting appears to have affected the representation of millions of voters.
\begin{figure}[htb!]
    \begin{center}
...
This is shown in figure \ref{fig:asymRank}, where the 2010 cycle stands out as having four states with expected asymmetries that exceeded anything seen previously.
These are Pennsylvania (19.7\%), North Carolina (19.5\%), Mississippi (18.8\%), and Georgia (16.7\%).
The high percentage asymmetry in Mississippi is party due to a small number of available seats in the state, but all four of these states showed a sudden increase in asymmetry in favor of Republicans after the 2010 redistricting.
Previous cycles may have had moderate asymmetries in a larger number of states, but in terms of extreme gerrymandering, there has been no time since the 70's that is comparable to the 2010 cycle.
