...
This amounts to counting the number of seats won by one party, then counting the number of seats the other party would win under a uniform partisan swing that reverses the statewide partisanship.
The specific asymmetry is then the difference between these two seat counts, divided by two.
This definition of partisan asymmetry simply requires the seat count to reverse along with a reversal in the popular vote.
...
However, we do not know the \emph{rate} of 'successes' (or the votes for an arbitrarily selected party), that is what we are trying to estimate.  
...
And the fact that it has a significantly higher efficiency gap than any of these computer-generated maps -- 10\% of which favor Democrats -- supports the exact opposite conclusion: \emph{Act 43} districts are, in fact, more "bizarrely shaped" than district plans with a \emph{lower} efficiency gap.
