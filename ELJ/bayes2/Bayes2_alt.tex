...
This is our error function to minimize.  We then write a computer program to test different values of our incumbency effect parameter, s, from 0.5 to 1.5, in increments of 0.01, and find the value that minimizes this error.  Furthermore, we plot this curve and visually confirm that it has no local minima besides the global minimum, thus confirming that our function is "well-behaved" in the sense that this approach to minimizing it produces a reasonable and reliable solution.

\subsection{potential further simplification (notes}
Since the composition of an invertible function with it's inverse produces the original, and f() is an invertible function,

\begin{equation}
vote(d,e) =f[ (f^{-1}(vote(d,e),s,i(d,e)) ,s,i(d,e) ]
\end{equation}

Therefore,

\begin{equation}
error = \sum_{d,e}{\left|  f[ avg(f^{-1}(vote(d,e'),s,i(d,e')) ,s,i(d,e) ] - f[ (f^{-1}(vote(d,e),s,i(d,e)) ,s,i(d,e) ])\right|}
\end{equation}

If f() is distributive, this implies

\begin{equation}
error = \sum_{d,e}{\left|  f[ avg(f^{-1}(vote(d,e'),s,i(d,e'))) - f^{-1}(vote(d,e),s,i(d,e))) ,s,i(d,e) ]\right|}
\end{equation}

Since f(x) has the same sign as x, the absolute value operator can be moved inside of f().

\begin{equation}
minimize \left( \sum_{d,e}{ \left| avg[ f^{-1}( vote(d,e'), s,i(d,e')) ] - f^{-1}( vote(d,e),s,i(d,e)) \right| } \right)
\end{equation}


And (actual vote balance - incumbency) is simply the incumbency-neutralized outcome.  And in equation 2) we see that district PVI is simply the average of that.  So what we are really minimizing here is the deviation of the incumbency-neutralized outcomes from their average.

\begin{equation}
error = \sum_{districts} \mid avg(vote_{actual} - incumbency) - (vote_{actual} - incumbency) \mid
\end{equation}
...
Where we wrote: vote balance + incumbency, we mean more generally incumbency(vote , strength)
Where we wrote: vote balance - incumbenc, we mean more generally incumbency-1(vote , strength)

The math all works out the same.  And in the end the final equation is:

\begin{equation}
minimize \left( \sum_{districts}{ \left| avg[ f^{-1}( vote_{actual}, strength) ] - f^{-1}( vote_{actual}, strength) \right| } \right)
\end{equation}
